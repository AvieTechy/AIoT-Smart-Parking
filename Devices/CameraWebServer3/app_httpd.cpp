...
const char* server_ip = "172.20.10.6";  // IP của ESP32 trung tâm
...
static esp_err_t stream_handler(httpd_req_t *req) {
    ...
    while (true) {
        ...
        if (detected != last_detected) { // Only send when detection status changes
            last_detected = detected;
            Serial.printf("[DEBUG] Trạng thái Wi-Fi: %d\n", WiFi.status());
            Serial.printf("[DEBUG] detected: %s\n", detected ? "true" : "false");
            if (WiFi.status() == WL_CONNECTED) {
                HTTPClient http;
                String serverUrl = "http://10.122.4.201/receive_detected";
        ...
        // Handle image capture and upload when a face is detected
        if (detected) {
            ...
            if (uploader.captureAndUpload(imageUrl)) {
                ...
                if (client.connect(server_ip, server_port)) {
                    ...
                    json += "\"isFace\":" + String(detected ? "true" : "false") + ",";
                    json += "\"url\":\"" + imageUrl + "\"";
                    json += "}";
